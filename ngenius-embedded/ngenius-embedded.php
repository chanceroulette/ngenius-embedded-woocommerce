<?php
/*
 * Plugin Name: N-Genius Embedded
 * Plugin URI: https://github.com/chanceroulette/ngenius-embedded-woocommerce/
...
if (version_compare(phpversion(), '8.0', '<')) {
    die("N-Genius Embedded requires PHP 8.0 or higher.");
}
...
define('NGENIUS_EMBEDDED_VERSION', '1.3.6'); // WRCS: DEFINED_VERSION.
define(
    'NGENIUS_EMBEDDED_URL',
    untrailingslashit(plugins_url(basename(plugin_dir_path(__FILE__)), basename(__FILE__)))
);
define('NGENIUS_EMBEDDED_PATH', untrailingslashit(plugin_dir_path(__FILE__)));

function ngenius_embedded_register_order_status()
{
...
add_action('init', 'ngenius_embedded_register_order_status');
...
function ngenius_cancel_order_handler(): void
{
    ...
    if ($order && current_user_can('view_order', $order_id)) {
        ...
        ngeniusEmbeddedRestoreCart($order_items);
    }
}
 ...
 * @return void
 */
function ngeniusEmbeddedRestoreCart(array $order_items): void
{
...
function ngenius_embedded_order_status($order_statuses)
{
    $statuses = NgeniusOrderStatuses::orderStatuses('N-Genius', 'ng');
...
add_filter('wc_order_statuses', 'ngenius_embedded_order_status');

global $wpdb;
define('NGENIUS_EMBEDDED_TABLE', $wpdb->prefix . 'ngenius_embedded');

function ngenius_embedded_table_install()
{
    $sql = 'CREATE TABLE IF NOT EXISTS `' . NGENIUS_EMBEDDED_TABLE . "` (
...
register_activation_hook(__FILE__, 'ngenius_embedded_table_install');

function ngenius_embedded_plugin_action_links($links)
{
    $plugin_links = array(
...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ngenius_embedded_plugin_action_links');

/*
 * The class itself, please note that it is inside plugins_loaded action hook
 */
add_action('plugins_loaded', 'ngenius_embedded_init_gateway_class');

/* end menu page */

function ngenius_embedded_print_errors()
{
    settings_errors('ngenius_error');
}

function ngenius_embedded_init_gateway_class()
{
    if (!class_exists('WC_Payment_Gateway')) {
        return;
    }
    include_once plugin_dir_path(__FILE__) . 'gateway/class-ngenius-embedded-gateway.php';
    if (class_exists('NgeniusEmbeddedGateway')) {
...
{
    $gateways[] = 'NgeniusEmbeddedGateway';
    return $gateways;
}

add_filter('woocommerce_payment_gateways', 'ngenius_embedded_add_gateway_class');

add_action('woocommerce_blocks_loaded', 'ngenius_embedded_woocommerce_blocks_support');

function ngenius_embedded_woocommerce_blocks_support()
{
    if (class_exists('Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType')) {
        require_once dirname(__FILE__) . '/gateway/class-ngenius-embedded-blocks-support.php';
        add_action(
            'woocommerce_blocks_payment_method_type_registration',
            function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
                $payment_method_registry->register(new NgeniusEmbeddedBlocksSupport);
            }
        );
    }
}

add_action('woocommerce_order_refunded', 'ngenius_embedded_after_refund', 10, 1);

function ngenius_embedded_after_refund($order_id): void
{
    $order = wc_get_order($order_id);
 ...
 * @return void
 */
function ngenius_embedded_declare_hpos_compatibility()
{
...
add_action('before_woocommerce_init', 'ngenius_embedded_declare_hpos_compatibility');

/**
 * Check if the intl extension is loaded and display an admin notice if not.
 */
function ngenius_embedded_check_intl_extension() {
    if (!extension_loaded('intl')) {
        add_action('admin_notices', 'ngenius_embedded_intl_extension_not_installed_notice');
    }
}
add_action('admin_init', 'ngenius_embedded_check_intl_extension');

/**
 * Display the admin notice.
 */
function ngenius_embedded_intl_extension_not_installed_notice() {
    ?>
    <div class="notice notice-error is-dismissible">
        <p>
            <?php esc_html_e('The intl PHP extension is required for N-Genius Embedded plugin to function correctly. Please install and enable the intl extension.', 'ngenius'); ?>
        </p>
