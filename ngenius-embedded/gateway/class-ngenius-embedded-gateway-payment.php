...
/**
 * NgeniusEmbeddedGatewayPayment class.
 */
class NgeniusEmbeddedGatewayPayment
{
    /**
     * N-Genius states
     */
    ...
    public function execute($order_ref): void
    {
        ...
        $config = new NgeniusEmbeddedGatewayConfig(new NgeniusEmbeddedGateway());
    ...
    {
        include_once dirname(__FILE__) . '/http/class-ngenius-embedded-gateway-http-abstract.php';
        include_once dirname(__FILE__) . '/config/class-ngenius-embedded-gateway-config.php';
        include_once dirname(__FILE__) . '/request/class-ngenius-embedded-gateway-request-token.php';
        include_once dirname(__FILE__) . '/http/class-ngenius-embedded-gateway-http-fetch.php';

        $gateway     = new NgeniusEmbeddedGateway();
        $order       = $this->fetch_order_by_reference($order_ref);
        $config       = new NgeniusEmbeddedGatewayConfig($gateway, $order);
        $token_class = new NgeniusEmbeddedGatewayRequestToken($config);
        $token       = $token_class->get_access_token();

        if ($token && !is_wp_error($token)) {
            $fetch_class = new NgeniusEmbeddedGatewayHttpFetch();
    ...
    public function update_table(array $data, int $nid, bool $abandoned_order): bool
    {
        ...
        $updated = $wpdb->update(NGENIUS_EMBEDDED_TABLE, $data, array('nid' => $nid));
